    ...
    <body >
      ...
      <div style="background-color: black;">
          <div class="row">
              ...
              <?php
                $sql = "SELECT Mo_Address FROM movie ORDER BY movie.Mo_Year DESC LIMIT 3,1";
                $result = mysqli_query($conn, $sql) or die('MySQL query error');
              ...
              <div class="col-md-6 clearfix" >
                <div style="padding: 20px;color: blanchedalmond;">
                  ...
                  <?php
                      $sql = "SELECT Mo_Grade FROM movie ORDER BY movie.Mo_Year DESC LIMIT 3,1";
                      $result = mysqli_query($conn, $sql) or die('MySQL query error');
                      $grade = mysqli_fetch_row($result);

                      $sql = "SELECT Mo_Dir FROM movie ORDER BY movie.Mo_Year DESC LIMIT 3,1";
                      $result = mysqli_query($conn, $sql) or die('MySQL query error');
                      $dir = mysqli_fetch_row($result);

                      $sql = "SELECT Mo_Tag FROM movie ORDER BY movie.Mo_Year DESC LIMIT 3,1";
                      $result = mysqli_query($conn, $sql) or die('MySQL query error');
                      $tag = mysqli_fetch_row($result);

                      $sql = "SELECT Mo_Year FROM movie ORDER BY movie.Mo_Year DESC LIMIT 3,1";
                      $result = mysqli_query($conn, $sql) or die('MySQL query error');
                      $time = mysqli_fetch_row($result);

                      $sql = "SELECT Mo_Info FROM movie ORDER BY movie.Mo_Year DESC LIMIT 3,1";
                      $result = mysqli_query($conn, $sql) or die('MySQL query error');
